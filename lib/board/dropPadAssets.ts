...
/**
 * Store a finished Dropbook in the Assets bin. Blob preview URLs are dropped —
 * they die with the page — so a reopened Dropbook shows labels and links until
 * its media is re-uploaded.
 */
export function placeDropbookInDropPadAssets(book: DropbookPlacement): DropPadAsset {
  const durable = (url?: string) => (url && !url.startsWith("blob:") ? url : undefined);
  const asset: DropPadAsset = {
    id: book.id,
    kind: "dropbook",
    title: book.title,
    createdAt: Date.now(),
    payload: {
      dropbook: {
        ...
        coverUrl: durable(book.coverUrl),
        ...
        pages: book.pages.map((page) => ({
          ...
          previewUrl: durable(page.previewUrl),
        })),
      },
    },
  };
