...
import { activityFromRoomShare, type RoomFeedItem } from "@/lib/board/rooms";
    ...
    return (
      <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-6">
        ...
        <p className="mt-1 text-sm text-white/50">
          Conversations, shared Drops, and future Live activity will gather here.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      {items.map((item) => {
        if (item.kind === "drop_share" && item.share) {
          const activity = activityFromRoomShare(item.share);
          if (activity) {
            return (
              <div key={item.id} className="overflow-hidden rounded-[1.5rem] border border-white/10">
                <div className="flex items-center gap-2 border-b border-white/10 bg-white/[0.04] px-4 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-emerald-100/80">
                  Shared Drop
                </div>
