...
function BankingPayDropsPanel({
    ...
    const [connectStatus, setConnectStatus] = useState<{
        ...
        loading: boolean;
        error: string | null;
    }>({ chargesEnabled: false, payoutsEnabled: false, detailsSubmitted: false, requirementsDue: [], loading: true, error: null });
    ...
    useEffect(() => {
        ...
        async function hydrateStripeStatus() {
            try {
                ...
                if (!uid) { if (!cancelled) setConnectStatus((s) => ({ ...s, loading: false })); return; }
                const { data: prof } = await supabase.from("profiles").select("board_style").eq("id", uid).maybeSingle();
                const style = prof?.board_style && typeof prof.board_style === "object" ? (prof.board_style as Record<string, any>) : {};
                const accountId = typeof style.stripeAccountId === "string" ? style.stripeAccountId.trim() : "";
                if (!accountId) { if (!cancelled) { setStripeAccountId(null); setConnectStatus((s) => ({ ...s, loading: false })); } return; }
                if (!cancelled) setStripeAccountId(accountId);
                const res = await fetch(`/api/paydrops/stripe/connect?accountId=${encodeURIComponent(accountId)}`);
                const data = await res.json().catch(() => null);
                if (!res.ok || !data?.ok) throw new Error(data?.error || "Could not load Stripe status.");
                if (!cancelled) setConnectStatus({ chargesEnabled: Boolean(data.chargesEnabled), payoutsEnabled: Boolean(data.payoutsEnabled), detailsSubmitted: Boolean(data.detailsSubmitted), requirementsDue: Array.isArray(data.requirementsDue) ? data.requirementsDue.map(String) : [], loading: false, error: null });
            } catch (error) {
                if (!cancelled) setConnectStatus((s) => ({ ...s, loading: false, error: error instanceof Error ? error.message : "Could not load Stripe status." }));
            }
        }
    ...
    const derivedStatus: BankingStatus = !stripeAccountId
        ...
        : connectStatus.requirementsDue.length > 0 || !connectStatus.detailsSubmitted
          ? "verification_needed"
          : connectStatus.payoutsEnabled ? "cash_out_available"
          : connectStatus.chargesEnabled ? "ready_for_pay_drops"
    ...
    const statusLabel = connectStatus.loading ? "Checking Stripe…" : BANKING_STATUS_LABELS[profile.status];
    const bankingConnected = Boolean(stripeAccountId);
    const cashOutDisabled = true;
    ...
    function cashOut() {
        if (typeof window !== "undefined") {
            window.alert(connectStatus.payoutsEnabled
